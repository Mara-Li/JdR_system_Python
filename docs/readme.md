...
- Les attaques mentales (parleur, liseur) : Elles nécessitent une défense avec un dé d'intelligence.
- Les attaques liés aux poisons et maladies (cheval pâle) : Elles nécessitent un dé de karma.
...
* **Dés** : Simplement le type de défense et les dés qui correspondent à l'attaque et la défense. De plus, vous pouvez cocher ou non si la personne possède une remise pour sa défense. Vous pouvez choisir si la défense est une endurance ou esquive. Le programme calculera automatiquement si l'esquive est esquivée (si vous avez la flemme).
...
* **Résultat** : Les PV restant (et le champ correspondant se met à jour) ainsi que les dégâts infligés. Vous pouvez donc enchainer sur plusieurs attaques sans avoir à reset entièrement le programme, si le défenseur est le même. 
...
*  Il n'est malheureusement pas possible de calculer les actions de soutien ou de malus avec le programme, vous devez donc multiplier vos bonus par vous même en suivant la table.
